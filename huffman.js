...
let cod = new Array();//массив для кодов 
let hashcodes = {};
for (let i = 0; i < alphLength; i++) {
    let m = i;
    cod[tree[m].letter] = '';
    while (tree[m].father != null) {
        cod[tree[i].letter] = tree[m].code + cod[tree[i].letter]; 
        m = tree[m].father; 
    }

    hashcodes[ cod[tree[i].letter] ] = tree[i].letter; 
}

//console.log(cod);
//console.log(hashcodes['000']);

let out = inputData;
//выводим получившуюся закодированную последовательность
for (i in cod) {
    out = out.replace(RegExp(i, 'g'), cod[i])
    }
...
for (i in out){
    str += out.charAt(i);
    if (hashcodes[str] != undefined) {
        output += hashcodes[str];
        str = '';
    }
