//node float.js tranl/calc input.txt output.txt
...
function back_conversate(output, mass3) {
    ...
    if (nord > 0){
        for (let i = 0; i < nord; i++) {
                mant3 += mass3[0].mantissa[i];
        }
        mant3 = '1' + mant3;
        mant3 = parseInt(mant3, 2).toString(10);
    }
    else if(nord < 0){
        mant3 = mass3[0].mantissa;
        mant3 = '1' + mant3;
        for (let i = 0; i < Math.abs(nord); i++) {
            mant3 = '0' + mant3;
        }
        mant3 = (mant3, 2).toString(10);
        mant3 = '0' + '.' + mant3;
    }
    numOut = '~'+' '+numOut + mant3;
}
...
if (arg[2] == 'calc') {
    ...
    back_conversate(output, mass3);
    output = output + ' '.repeat(8) + numOut;
    fs.writeFileSync(arg[4], output);
}
